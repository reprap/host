...
public class RrGraphics {
    ...
    public void add(String gCode) {
        ...
        RrRectangle box = new RrRectangle(new RrInterval(0, 200), new RrInterval(0, 200)); // Default is entire plot
                                                                                           // area
    ...
    private Rr2Point transform(Rr2Point p) {
        return new Rr2Point(p_0.x() + scale * p.x(), (double) frameHeight - (p_0.y() + scale * p.y()));
    }
    ...
    private Rr2Point iTransform(int x, int y) {
        return new Rr2Point(((double) x - p_0.x()) / scale, ((double) (frameHeight - y) - p_0.y()) / scale);
    }
    ...
    private void plot(Rr2Point p) {
        ...
        g2d.drawLine((int) Math.round(pos.x()), (int) Math.round(pos.y()), (int) Math.round(a.x()),
            (int) Math.round(a.y()));
        pos = a;
    }
