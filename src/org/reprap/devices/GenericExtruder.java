...
public abstract class GenericExtruder implements Extruder {
    ...
    /**
     * Flag to decide if the machine implements 4D extruding (i.e. it processes extrude lengths along with X, Y, and Z
    ...
    /**
     * The extrusion height in Z TODO: Should this be a machine-wide constant? - AB
     */
    ...
    /**
     * One physical extruder can have several of this class attached to it with different myExtruderIDs.
     * physicalExtruder is a unique integer for each actual extruder, derived from the old SNAP address of the extruder,
    ...
    /**
     * Allow others to set our extrude length so that all logical extruders talking to one physical extruder can use the
     * same length instance.
     * 
     * @param e
     */
    ...
    /**
     * Start the extruder motor at a given speed. In the old scheme, this ranges from 0 to 255 but is scaled by maxSpeed
     * and t0, so that 255 corresponds to the highest permitted speed. It is also scaled so that 0 would correspond with
     * the lowest extrusion speed.
     * 
     ...
     * getAngleSpeedFactor()). (non-Javadoc)
     * 
    ...
    /**
     * At the top and bottom return the fine width; in between return the braod one. If the braod one is negative, just
     * do fine.
     */
    ...
    /**
     * Get how much extrudate is deposited in a given time (in milliseconds) currentSpeed is in mm per minute. Valve
     * extruders cannot know, so return 0.
     * 
     * @param time
     *            (ms)
     * @return
    ...
    /**
     * Get how much extrudate is deposited for a given xy movement currentSpeed is in mm per minute. Valve extruders
     * cannot know, so return 0.
     * 
     * @param xyDistance
     *            (mm)
     * @param xyFeedrate
