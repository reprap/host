...
public class LayerProducer {
    ...
    /**
     * @param first
     *            First point, the end of the line segment to be plotted to from the current position.
     * @param second
     *            Second point, the end of the next line segment; used for angle calculations
     * @param turnOff
     *            True if the extruder should be turned off at the end of this segment.
     * @throws Exception
     */
    private void plot(Rr2Point first, Rr2Point second, boolean stopExtruder, boolean closeValve) throws Exception {
        ...
        if (speedUpLength > 0) {
            ...
            // printer.setFeedrate(printer.getExtruder().getAngleFeedrate());
            printer
                .printTo(ss.p3.x(), ss.p3.y(), z, printer.getExtruder().getAngleFeedrate(), stopExtruder, closeValve);
            // pos = ss.p3;
     ...
     * @throws Exception
     */
    private void move(Rr2Point first, Rr2Point second, boolean startUp, boolean endUp, boolean fast) throws Exception {
        Printer printer = layerConditions.getPrinter();
    ...
    private void plot(RrPolygon p, boolean firstOneInLayer) throws Exception {
        ...
        if (acc)
            p.setSpeeds(att.getExtruder().getSlowXYFeedrate(), p.isClosed() ? outlineFeedrate : infillFeedrate, att
                .getExtruder().getMaxAcceleration());
    ...
    /**
     * Master plot function - draw everything. Supress border and/or hatch by setting borderPolygons and/or
