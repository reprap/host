...
public class Producer {
    ...
    public Producer(Printer pr, RepRapBuild builder) throws Exception {
        ...
        gp = new RrRectangle(new Rr2Point(gp.x().low() - 6, gp.y().low() - 6), new Rr2Point(gp.x().high() + 6, gp.y()
            .high() + 6));
        ...
        layerRules = new LayerRules(pr, modZMax, modZMax + foundationLayers * stepZ, modLMax, modLMax
            + foundationLayers, true, gp);
    }
    ...
    /**
     * NB - this does not call wait - this is a purely interactive function and does not control the machine
     * 
     */
    ...
    private void fillFoundationRectangle(Printer reprap, RrRectangle gp) throws Exception {
        ...
        RrPolygonList h[] = { shield,
            bg.hatch(layerRules.getHatchDirection(e), layerRules.getHatchWidth(e), bg.attribute()) };
        ...
        for (int extruder = 0; extruder < reprap.getExtruders().length; extruder++) {
            ...
            if (thisExtruder > lastExtruder) {
                ...
                if (thisExtruder - lastExtruder != 1) {
                    Debug.e("Producer.produceAdditiveTopDown(): Physical extruders out of sequence: " + lastExtruder
                        + " then " + thisExtruder);
            ...
            for (int stl = 0; stl < allSTLs.size(); stl++) {
                ...
                for (int physicalExtruder = 0; physicalExtruder < allPolygons.length; physicalExtruder++) {
                    if (tempBorderPolygons[physicalExtruder].size() > 0) {
                        ...
                        tempBorderPolygons[physicalExtruder] = tempBorderPolygons[physicalExtruder].nearEnds(
                            startNearHere, false, -1);
                        if (tempBorderPolygons[physicalExtruder].size() > 0) {
                            RrPolygon last = tempBorderPolygons[physicalExtruder]
                    ...
                    if (tempFillPolygons[physicalExtruder].size() > 0) {
                        ...
                        tempFillPolygons[physicalExtruder] = tempFillPolygons[physicalExtruder].nearEnds(startNearHere,
                            false, -1);
                        if (tempFillPolygons[physicalExtruder].size() > 0) {
                            RrPolygon last = tempFillPolygons[physicalExtruder]
    ...
    /**
     * The total distance moved is the total distance extruded plus plus additional movements of the extruder when no
     * materials was deposited
     * 
    ...
    public double getTotalVolumeExtruded() {
        return layerRules.getPrinter().getTotalDistanceExtruded()
            * layerRules.getPrinter().getExtruder().getExtrusionHeight()
