...
/**
 * This stores a set of facts about the layer currently being made, and the rules for such things as infill patterns,
 * support patterns etc.
 */
public class LayerRules {
        ...
        if (es.length > 1) {
            for (int i = 1; i < es.length; i++) {
                ...
                if (Math.abs(es[i].getExtrusionHeight() - zStep) > Preferences.tiny())
                    Debug.e("Not all extruders extrude the same height of filament: " + zStep + " and "
                        + es[i].getExtrusionHeight());
            }
        }
    ...
    public void setFirstAndLast(RrPolygonList[] pl) {
        ...
        lastPoint[modelLayer] = pl[pl.length - 1].polygon(pl[pl.length - 1].size() - 1).point(
            pl[pl.length - 1].polygon(pl[pl.length - 1].size() - 1).size() - 1);
        lastExtruder[modelLayer] = pl[pl.length - 1].polygon(pl[pl.length - 1].size() - 1).getAttributes()
            .getExtruder().getID();
    }
    ...
    public void setLayingSupport(boolean lf) {
        layingSupport = lf;
    }
     ...
     * 
     * Model: Alternate even then odd (which can be set to the same angle if wanted).
     * 
     * @return
     */
     ...
     * 
     * Foundation: The foundation width for all but... ...the penultimate foundation layer, which is half that and..
