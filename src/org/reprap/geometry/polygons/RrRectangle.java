...
public class RrRectangle {
    ...
    public RrRectangle offset(double dist) {
        return new RrRectangle(new RrInterval(x.low() - dist, x.high() + dist), new RrInterval(y.low() - dist, y.high()
            + dist));
    }
    ...
    public RrRectangle translate(Rr2Point p) {
        return new RrRectangle(new RrInterval(x.low() + p.x(), x.high() + p.x()), new RrInterval(y.low() + p.y(),
            y.high() + p.y()));
    }
