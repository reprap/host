...
public class RrPolygon {
    ...
    /**
     * Something has been done to the polygon that may require its extrude and valve endings to be updated
     */
    ...
    public void set(int i, Rr2Point p) {
        ...
        box.expand(p); // Note if the old point was on the convex hull, and the new one is within, box will be too big
                       // after this
        updateExtrudeValveEnd();
    }
    ...
    /**
     * Put a new polygon on the end (N.B. Attributes of the new polygon are ignored)
     * 
     * @param p
     */
    ...
    /**
     * Put a new polygon in the middle (at vertex k, which will be at the end of the inserted polygon afterwards). (N.B.
     * Attributes of the new polygon are ignored)
     * 
     * @param k
     * @param p
     */
    public void add(int k, RrPolygon p) {
        ...
        if (speeds != p.speeds) {
            Debug
                .e("Rr2Point.add(): attempt to add a polygon to another polygon when one has speeds and the other doesn't.");
            return;
        }
        for (int i = 0; i < p.size(); i++) {
            if (speeds != null)
    ...
    /**
     * Find the nearest vertex on this polygon to any on polygon p, reorder p so that its nearest is its first one, then
     * merge that polygon into this one. The reordering is only done if the distance^2 is less than linkUp. If no
     * reordering and merging are done false is returned, otherwise true is returned.
     * 
     * @param p
     * @param linkUp
     * @return
    ...
    /**
     * Find the index of the polygon point that has the maximal parametric projection onto a line.
     * 
     * @param ln
     * @return
     */
    ...
    /**
     * Backtrack a given distance, inserting a new point there and set extrudeEnd to it. If extrudeEnd is already set,
     * backtrack from that.
     * 
     * @param distance
     *            to backtrack
    ...
    /**
     * Backtrack a given distance, inserting a new point there and set valveEnd to it. If valveEnd is already set,
     * backtrack from that.
     * 
     * @param distance
     *            to backtrack
    ...
    /**
     * Simplify a polygon by deleting points from it that are closer than d to lines joining other points NB - this
     * ignores speeds
     * 
     * @param d
    ...
    /**
     * Remove solitary edges that are shorter than tiny from the polygon if they are preceeded and followed by gap
     * material.
     * 
     * @param tiny
    ...
    /**
     * Offset (some of) the points in the polygon to allow for the fact that extruded circles otherwise don't come out
     * right. See http://reprap.org/bin/view/Main/ArcCompensation. If the extruder for the polygon's arc compensation
     * factor is 0, return the polygon unmodified.
     * 
     * This ignores speeds
     * 
     * @param es
    ...
    private RrCSG toCSGRecursive(List<Integer> a, int level, boolean closed, int[] flags) {
        ...
        while (section != null) {
            if (level % 2 == 1)
                hull = RrCSG.intersection(hull, toCSGRecursive(section, level, false, flags));
            else
                hull = RrCSG.union(hull, toCSGRecursive(section, level, false, flags));
