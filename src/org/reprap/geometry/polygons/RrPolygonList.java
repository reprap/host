...
/**
 * tree - class to hold lists to build a containment tree (that is a representation of which polygon is inside which,
 * like a Venn diagram).
 */
class treeList {
    /**
    ...
    /**
     * Do a depth-first walk setting parents. Any node that appears in more than one list should have the deepest
     * possible parent set as its parent, which is what we want.
     * 
     * @param node
     */
...
/**
 * RrPolygonList: A collection of 2D polygons List of polygons class. This too maintains a maximum enclosing rectangle.
 */
public class RrPolygonList {
    ...
    /**
     * Simplify all polygons by length d N.B. this may throw away small ones completely
     * 
     * @param d
    ...
    /**
     * Re-order and (if need be) reverse the order of the polygons in a list so the end of the first is near the start
     * of the second and so on. This is a heuristic - it does not do a full travelling salesman... This deals with both
     * open and closed polygons, but it only allows closed ones to be re-ordered if reOrder is true. If any point on a
     * closed polygon is closer to any point on any other than sqrt(linkUp), the two polygons are merged at their
     * closest points. This is suppressed by setting linkUp negative.
     * 
     * @param startNearHere
     * @param reOrder
     * @param linkUp
    ...
    /**
     * Take all the polygons in a list, both open and closed, and reorder them such that accessible points on any that
     * have a squared distance less than linkUp to accessible points on any others are joined to form single polygons.
     * 
     * For an open polygon the accessible points are just its ends. For a closed polygon all its points are accessible.
     * 
     * This is a fairly radical remove in-air movement strategy.
     * 
     * All the polygons in the list must be plotted with the same physical extruder (otherwise it would be nonsense to
     * join them). It is the calling function's responsibility to make sure this is the case.
     * 
     * @param linkUp
     */
        ...
        for (int i = 0; i < size() - 1; i++) {
            ...
            for (int j = i + 1; j < size(); j++) {
                ...
                if (!myPolygon.isClosed() && !itsPolygon.isClosed()) {
                    ...
                    d2 = Rr2Point.dSquared(myPolygon.point(myPolygon.size() - 1),
                        itsPolygon.point(itsPolygon.size() - 1));
                    if (d2 < d) {
                        reverseMe = false;
                        reverseIt = true;
                        d = d2;
    ...
    /**
     * Remove polygon pol from the list, replacing it with two polygons, the first being pol's vertices from 0 to st
     * inclusive, and the second being pol's vertices from en to its end inclusive. It is permissible for st == en, but
     * if st > en, then they are swapped.
     * 
    ...
    /**
     * Search a polygon list to find the nearest point on all the polygons within it to the point p. If omit is
     * non-negative, ignore that polygon in the search.
     * 
    ...
    /**
     * This assumes that the RrPolygonList for which it is called is all the closed outline polygons, and that hatching
     * is their infill hatch. It goes through the outlines and the hatch modifying both so that that outlines actually
     * start and end half-way along a hatch line (that half of the hatch line being deleted). When the outlines are then
     * printed, they start and end in the middle of a solid area, thus minimising dribble.
     * 
     * The outline polygons are re-ordered before the start so that their first point is the most extreme one in the
     * current hatch direction.
     * 
    ...
    public void middleStarts(RrPolygonList hatching, LayerRules lc, BooleanGridList slice) {
        for (int i = 0; i < size(); i++) {
            ...
            if (ex.getMiddleStart()) {
                ...
                PolPoint pp = hatching.ppSearch(start, -1, outline.getAttributes().getExtruder()
    ...
    /**
     * Offset (some of) the points in the polygons to allow for the fact that extruded circles otherwise don't come out
     * right. See http://reprap.org/bin/view/Main/ArcCompensation.
     * 
     * @param es
     */
    ...
    /**
     * Take a list of CSG expressions, each one corresponding with the entry of the same index in this class, classify
     * each as being inside other(s) (or not), and hence form a single CSG expression representing them all.
     * 
     * @param csgPols
     * @param polAttributes
