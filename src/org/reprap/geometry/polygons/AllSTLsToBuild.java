 ...
 * 
 * An STLObject may consist of items from several STL files, possible of different materials. But they are all tied
 * together relative to each other in space.
 * 
 * @author Adrian
 * 
 */
...
public class AllSTLsToBuild {
    ...
    /**
     * Reorder the list under user control. The user sends items from the old list one by one. These are added to a new
     * list in that order. When there's only one left that is added last automatically.
     * 
     * Needless to say, this process must be carried through to completion. The function returns true while the process
     * is ongoing, false when it's complete.
     * 
     * @param st
     * @return
     */
    ...
    /**
     * Freeze the list - no more editing. Also compute the XY box round everything. Also compute the individual plan
     * boxes round each STLObject.
     */
    private void freeze() {
        if (frozen)
            return;
            ...
            while ((land1 = findLand(landPattern)) != null) {
                ...
                if (cen2 == null) {
                    ...
                    result.hatchedPolygons.add(bridge.hatch(
                        layerConditions.getHatchDirection(bridge.attribute().getExtruder()), bridge.attribute()
                    ...
                    result.hatchedPolygons.add(bridge.hatch(new RrHalfPlane(new Rr2Point(0, 0), bridgeDirection),
                        bridge.attribute().getExtruder().getExtrusionInfillWidth(), bridge.attribute()));
    ...
    public RrPolygonList computeInfill(int stl, LayerRules layerConditions) {
        ...
        if (layerConditions.getModelLayer() < 2
            || layerConditions.getModelLayer() > layerConditions.getModelLayerMax() - 3) {
        ...
        BooleanGridList lands = BooleanGridList.intersections(infill.bridges,
     ...
     * @return
     */
    public RrPolygonList computeOutlines(int stl, LayerRules layerConditions, RrPolygonList hatchedPolygons,
        boolean shield) {
        // No more additions or movements, please
    ...
    private BooleanGridList slice(int stl, int layer, LayerRules layerRules) {
        ...
        for (extruderID = 0; extruderID < extruders.length; extruderID++) {
            if (extruders[extruderID].getID() != extruderID)
                Debug.e("AllSTLsToBuild.slice(): extruder " + extruderID + "out of sequence: "
        ...
        for (extruderID = 0; extruderID < edges.length; extruderID++) {
            ...
            if (pgl.size() > 0) {
                ...
                // quickly as each is in the same rectangle so the bit patterns match exactly. But it does use more
                // memory.
    ...
    /**
     * Add the edge where the plane z cuts the triangle (p, q, r) (if it does). Also update the triangulation of the
     * object below the current slice used for the simulation window.
     * 
     * @param p
     * @param q
     * @param r
