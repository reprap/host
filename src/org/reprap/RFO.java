...
public class RFO {
    ...
    class XMLIn extends DefaultHandler {
        ...
        public void startElement(String uri, String name, String qName, org.xml.sax.Attributes atts) {
            ...
            if (element.equalsIgnoreCase("reprap-fab-at-home-build")) {
                ...
                if (!filetype.equalsIgnoreCase("application/sla"))
                    Debug.e("XMLIn.startElement(): unreconised object file type (should be \"application/sla\"): "
                        + filetype);
        ...
        public void endElement(String uri, String name, String qName) {
            ...
            if (element.equalsIgnoreCase("reprap-fab-at-home-build")) {
                ...
                org.reprap.Attributes att = stl.addSTL("file:" + rfoDir + location, null, Preferences.unselectedApp(),
                    firstSTL);
                if (firstSTL == null)
                    firstSTL = stl;
                ...
                if (rowNumber != 4)
                    Debug.e("XMLIn.endElement(): incomplete Transform3D matrix - last row number is not 4: "
                        + rowNumber);
                transform = new Transform3D(mElements);
            } else if (element.equalsIgnoreCase("row")) {
                rowNumber++;
    ...
    /**
     * The constructor is the same whether we're reading or writing. fn is where to put or get the rfo file from. as is
     * all the things to write; set that null when reading.
     * 
     * @param fn
     * @param as
     */
    ...
    /**
     * Copy each unique STL file to the temporary directory. Files used more than once are only copied once.
     * 
     */
    ...
    private void createLegend() {
        ...
        for (int i = 0; i < astl.size(); i++) {
            ...
            for (int subObj = 0; subObj < stlo.size(); subObj++) {
                xml.push("file location=\"" + stlName(stlo.getUnique(subObj))
    ...
    /**
     * The entire temporary directory with the legend file and ann the STLs is complete. Compress it into the required
     * rfo file using zip. Note we delete the temporary files as we go along, ending up by deleting the directory
     * containing them.
     * 
     */
