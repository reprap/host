...
public class GCodeRepRap extends GenericRepRap {
    ...
    public GCodeRepRap() throws Exception {
        ...
        // FirmwareConfig = FirmwareConfig.replace('\\','\n'); //make it easier to read for humans if it has
        // "continuations"
    ...
    public void moveTo(double x, double y, double z, double feedrate, boolean startUp, boolean endUp) throws Exception {
        ...
        if (zMove && xyMove)
            Debug.d("GcodeRepRap.moveTo(): attempt to move in X|Y and Z simultaneously: (x, y, z) = (" + x + ", " + y
                + ", " + z + ")");
    ...
    public void singleMove(double x, double y, double z, double feedrate) {
        ...
        if (zMove && xyMove)
            Debug.d("GcodeRepRap.singleMove(): attempt to move in X|Y and Z simultaneously: (x, y, z) = (" + x + ", "
                + y + ", " + z + ")");
        ...
        try {
            ...
            if (xyMove) {
                ...
                VelocityProfile vp = new VelocityProfile(s, getExtruder().getSlowXYFeedrate(), feedrate, getExtruder()
            ...
            if (zMove) {
                VelocityProfile vp = new VelocityProfile(Math.abs(dz), getSlowZFeedrate(), feedrate,
                    getSlowZFeedrate(), getMaxZAcceleration());
                double s = 1;
                if (dz < 0)
                    s = -1;
                switch (vp.flat()) {
