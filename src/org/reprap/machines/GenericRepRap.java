...
public abstract class GenericRepRap implements CartesianPrinter {
    ...
    /**
     * Extrude for the given time in milliseconds, so that polymer is flowing before we try to move the extruder. But
     * first take up the slack from any previous reverse.
     */
    ...
    private void checkCoordinates(double x, double y, double z) {
        try {
            if (x > Preferences.loadGlobalDouble("WorkingX(mm)") || x < 0)
                Debug.e("Attempt to move x to " + x + " which is outside [0, "
                    + Preferences.loadGlobalDouble("WorkingX(mm)") + "]");
            if (y > Preferences.loadGlobalDouble("WorkingY(mm)") || y < 0)
                Debug.e("Attempt to move y to " + y + " which is outside [0, "
                    + Preferences.loadGlobalDouble("WorkingY(mm)") + "]");
            if (z > Preferences.loadGlobalDouble("WorkingZ(mm)") || z < 0)
                Debug.e("Attempt to move z to " + z + " which is outside [0, "
                    + Preferences.loadGlobalDouble("WorkingZ(mm)") + "]");
        } catch (Exception e) {
        }
    }
    ...
    /**
     * Deals with all the actions that need to be done between one layer and the next. THIS FUNCTION MUST NOT MAKE THE
     * REPRAP MACHINE DO ANYTHING (LIKE MOVE).
     * 
     * @param lc
     */
    ...
    protected void segmentPause() {
        ...
        ContinuationMesage msg = new ContinuationMesage(null, "A new segment is about to be produced");
    ...
    /**
     * Display a message indicating a layer is about to be printed and wait for the user to acknowledge
     */
    protected void layerPause() {
        ContinuationMesage msg = new ContinuationMesage(null, "A new layer is about to be produced");
        // ,segmentPauseCheckbox, layerPauseCheckbox);
        msg.setVisible(true);
        try {
            synchronized (msg) {
