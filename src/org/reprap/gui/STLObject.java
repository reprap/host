...
/**
 * Class for holding a group (maybe just 1) of 3D objects for RepRap to make. They can be moved around on the build
 * platform en mass, but not moved relative to each other, so they can represent an assembly made from several different
 * materials.
 * 
 * @author adrian
 * 
...
public class STLObject {
    ...
    /**
     * Load an STL object from a file with a known offset (set that null to put the object bottom-left-at-origin) and
     * set its appearance
     * 
     * @param location
     * @param offset
    ...
    /**
     * Actually load the stl file and set its attributes. Offset decides where to put it relative to the origin. If
     * lastPicked is null, the file is loaded as a new independent STLObject; if not it is added to lastPicked and
     * subsequently is subjected to all the same transforms, so they retain their relative positions. This is how
     * multi-material objects are loaded.
     * 
     * @param location
     * @param att
     * @param offset
    ...
    /**
     * Find how to move the object by actually changing all its coordinates (i.e. don't just add a transform). Also
     * record its size.
     * 
     * @param child
     * @param offset
    ...
    /**
     * method to recursively set the user data for objects in the scenegraph tree we also set the capabilites on Shape3D
     * objects required by the PickTool
     * 
     * @param value
     * @param me
     */
