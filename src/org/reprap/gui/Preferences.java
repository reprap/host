 ...
 * 
 * Preference keys either start with the string "Extruder" followed by a number and an underscore (that is, they look
 * like "Extruder3_temp(C)") in which case they are assumed to be a characteristic of the extruder with that number; or
 * they don't, in which case they are assumed to be global characteristics of the entire machine.
 * 
 * The keys should end with their dimensions: "Extruder3_temp(C)", "Axis2Scale(steps/mm)", but regrettably can't contain
 * un-escaped space characters (see java.util.Properties).
 * 
 * Some weak type checking is done to prevent obvious crassness being put in the edit boxes. This is done at save time
 * and prevents the junk being written, but doesn't give a chance to correct it.
 * 
 * Extensively adapted from Simon's old version by Adrian to construct itself from the preferences file.
 * 
 */
...
public class Preferences extends JFrame {
    ...
    /**
     * Constructor loads all the information from the preferences file, converts it into arrays of JPanels and
     * JTextFields, then builds the menus from them.
     * 
     * @param frame
     */
    public Preferences() {
    ...
    /**
     * Take an array of labels and use their string values as keys to look up the corresponding values. Make those into
     * an array of editable boxes.
     * 
     * @param a
     * @return
     */
     ...
     * 
     * There must be a better way to do this; also this doesn't allow for 1.3e-5...
     * 
     * @param s
     * @return
     */
