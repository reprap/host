...
public class BotConsoleFrame extends javax.swing.JFrame {
    ...
    private void updatePanels() 
    {
        if (!org.reprap.Main.inFormDesignMode()) {
            int currentExtruder = org.reprap.Main.getPrinter().getExtruder()                    .getID();

            try {
                org.reprap.Main.getPrinter().selectExtruder(exPanelNumber);
            } catch (Exception e) {
                handleException(e);
            }
            extruderPanels[exPanelNumber].refreshTemperature();
            try {
                org.reprap.Main.getPrinter().selectExtruder(currentExtruder);
            } catch (Exception e) {
                handleException(e);
            }
